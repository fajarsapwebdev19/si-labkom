<?php
    ...
    if(isset($_POST['update']))
    {
        if(isset($_POST['id']))
        {
            ...
            $tmp_pc = $_FILES['foto_pc']['tmp_name'];
            $ex = array('png','jpg','jpeg');
            $extend = pathinfo($foto_pc, PATHINFO_EXTENSION);
            $size = $_FILES['foto_pc']['size'];
            $rename = rand().'_'.$foto_pc;
            ...
            if(empty($_FILES['foto_pc']['name']))
            {
                ...
                if(!in_array($extend, $ex))
                {
                    ...
                    if($size > 3000000)
                    {
                        ...
                        move_uploaded_file($tmp_pc, $dir);

                        $update = mysqli_query($koneksi, "UPDATE pc SET foto_pc='$rename' WHERE id='$id'");
